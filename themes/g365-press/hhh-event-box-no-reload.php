...
/**
 * Template Name: hhh-scouting-services-box-scores
 * Author: Chritsian Jimenez
 * Version: 1.0
 * Description: My page that i use in order to be able to filter the event standings without having to reload the whole page.
 */
  ...
  $single_event = $_POST['single_event'];
// echo('new page: ' . $single_event);
...
//       echo g365_submenu_type(array($wp_query->query_vars["pg_type"], $select_year), 2); 
      ...
      }else if($single_event === 'false'){
        echo g365_submenu_type(['post_gp_lv'=>$select_group, 'post_year'=>$select_year, 'lv_play'=>$select_lv_play], 14);
      }
...
// echo(' event_id: ' . $event_id . ' //$team_id: ' . $team_id . ' //$opponent_id: ' . $opponent_id . ' //is_dcp_ev: ' . $arg['is_dcp_ev'] . ' //$select_year: ' . $select_year . ' //$select_gp_div: ' . $select_gp_div . ' //$select_group: ' . $select_group . ' //$select_lv_play: ' . $select_lv_play . ' //$player_id: ' . $player_id . '//');
  $club_team_datas = cj_g365_club_team_stat($event_id = null, $team_id = null, (empty($arg['is_dcp_ev']) ? '' : $arg['is_dcp_ev']), $opponent_id = null, $select_year, 9, array($select_gp_div, null, 'is_standing_only', null, null, $select_group, 'is_main_ts'=>true, 'level_of_play'=>$select_lv_play, 'is_dcp_ev'=>$player_id, 'is_unlocked_dcp_ev'=>true, 'is_dcp'=>true));
...
<!--     <tr class="text-center slb_more_list" id="<?php //echo $index ?>" style="font-weight:bold;cursor:pointer;">
      <td class="buttonization">
        <div>
          <a style="color:#ffffff" href="<?php echo get_site_url(); ?>/club-team-standing/<?php echo empty($_POST['group_lv']) ? 'youth-girls' :$_POST['group_lv']; ?>/<?php $lv_url = strtolower(str_replace(array(" / ","/"," "), array("-","-","-"), $key_level[$level_index])); echo $lv_url ?>/<?php echo $level_index?>/?y=<?php echo $select_year. $lv_of_lv_pg ?>"><span>View Complete Leaders</span></a>
        </div>
      </td>
    </tr> -->
    </tbody>
