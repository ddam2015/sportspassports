<?php 
  $select_group = $_POST['group_lv']; //grade
  $select_year = $_POST['g365_year']; //year
  $level_of_play = $_POST['lv_of_play']; //level
  $select_ev = $arg['ev_id']; //empty
  if(!isset($select_year)){ $select_year = wp_date('Y'); }
  if(!isset($select_group)){ $select_group = '17U'; }
  $g365_team_standings = g365_team_standing(['post_year'=>$select_year, 'post_gp_lv'=>$select_group, 'is_year'=>$is_year, 'is_gp_lv'=>$is_gp_lv, 'is_dcp_ev'=>$select_ev, 'is_dcp_only'=>true, 'post_ros_dvs'=>$level_of_play]);
  echo($g365_team_standings);
$g365_team_standings = json_decode( json_encode($g365_team_standings), true); echo $g365_team_standings[0]; if(!empty($g365_team_standings[1])):/*main*/ foreach($g365_team_standings[1] as $level_index => $club_team_data): /*foreach-main*/?>
  <div id="dialong_div"></div>
  <div class="max-width-1200">
    <h5><?php echo $g365_team_standings[3][$level_index]. ' ' .$level_of_play ?></h5>
    <table class="cell cts_tb dcp-team-standings">
